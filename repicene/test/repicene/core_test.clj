...
(deftest instructions
  (testing "ld a,l"
    (let [cpu (-> (compile-prog ["ld a,l"])
                  (cpu/new-cpu)
                  (cpu/set-dword-register :HL 0x0B8F))]
      (is (= 0x8F (get-source a (cpu-cycle cpu))))))
  (testing "ld a,h"
    (let [cpu (-> (compile-prog ["ld a,h"])
                  (cpu/new-cpu)
                  (cpu/set-dword-register :HL 0x0B8F))]
      (is (= 0x0B (get-source a (cpu-cycle cpu))))))

  (testing "ldh [FF00+n],a"
    (let [cpu (-> (compile-prog [["ldh [FF00+n],a", 0x42]])
                  (cpu/new-cpu)
                  (cpu/set-word-at 0xFF42 0xAA)
                  (cpu/set-dword-register :AF 0xBB00))]
      (is (= 0xAA (cpu/word-at cpu 0xFF42)))
      (is (= 0xAA (get-source <FF00+n> cpu)))
      (is (= 0xBB (cpu/word-at (cpu-cycle cpu) 0xFF42)))))
  (testing "ld a,0x77"
    (let [cpu (-> (compile-prog [["ld a,0x77", 0x77]])
                  (cpu/new-cpu)
                  (cpu/set-dword-register :AF 0xBB00))]
      (is (= 0xBB (get-source a cpu)))
      (is (= 0x77 (get-source a (cpu-cycle cpu))))))
  ...
  (testing "push hl"
    (let [cpu (-> (compile-prog ["push hl"])
                  (cpu/new-cpu)
                  (cpu/set-dword-register :HL 0xABCD)
                  (cpu/set-dword-register :SP 0xE000))]
      (is (= 0xABCD (get-source hl cpu)))
      (is (= 0xE000 (get-source sp cpu)))
      (let [cpu-afer-push (cpu-cycle cpu)]
        (is (= 0xDFFE (get-source sp cpu-afer-push)))
        (is (= 0xABCD (dword-at cpu-afer-push 0xDFFE))))))
  (testing "cp 0x90"
    (let [cpu (-> (compile-prog ["cp 0x90"])
                  (cpu/new-cpu)
                  (cpu/set-dword-register :AF 0xAA00))]
      (is (= 0xAA (get-source a cpu)))
      (let [cpu-afer-cp (cpu-cycle cpu)]
        (is (= 0xAA (get-source a cpu-afer-cp))))))
  (testing "ld l,[hl]"
    (let [cpu (-> (compile-prog ["ld l,[hl]"])
                  (cpu/new-cpu)
                  (cpu/set-dword-register :HL 0x8000)
                  (cpu/set-word-at 0x8000 0xEE))]
      (is (= 0x8000 (get-source hl cpu)))
      (is (= 0xEE (cpu/word-at cpu 0x8000)))
      (let [cpu-after-ld (cpu-cycle cpu)]
        (is (= 0xEE (get-source l cpu-after-ld))))))
  (testing "sub a,0x05"
    (let [cpu (-> (compile-prog ["sub a,0x05"])
                  (cpu/new-cpu)
                  (cpu/set-dword-register :AF 0x0300))]
      (is (= 0x03 (get-source a cpu)))
      (is (= 0xFE (get-source a (cpu-cycle cpu)))))))
...
(def invalid-opcodes #{0xD3 0xDB 0xDD 0xE3 0xE4 0xEB 0xEC 0xED 0xF4 0xFC 0xFD})
(def blank-cpu (cpu/new-cpu (complete-program [])))
(deftest completeness
  (testing "all instruction set is implemented"
    (doseq [i (range 0x100)
            :let [instr (decoder i)]]
      (is (some? instr) (str "instruction " (hex8 i) " is decoded"))
      (when (not (invalid-opcodes i))
        (is (s/cpu? (exec instr blank-cpu)))
        (is (string? (print-assembly instr blank-cpu))))))
  (testing "all extra instructions can be decoded"
    (doseq [i (range 256)
            :let [instr (extra-decoder i)]]
      (is (some? instr) (str "instruction " (hex8 i) " is decoded"))
      (is (string? (print-assembly instr blank-cpu)))
      (is (s/cpu? (exec instr blank-cpu))))))
